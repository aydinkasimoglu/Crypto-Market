...
class TransactionDatabase {
  ...
  Future<CryptoTransaction> getTransaction(int id) async {
    ...
    final result = await db.query(tableCryptoTransaction,
        columns: CryptoTransactionFields.values, where: '${CryptoTransactionFields.id} = ?', whereArgs: [id]);
    return result.isNotEmpty ? CryptoTransaction.fromJson(result.first) : throw Exception('Transaction $id not found');
  }
  ...
  Future<List<CryptoTransaction>> getTransactions() async {
    final db = await instance.database;
    final result = await db.query(tableCryptoTransaction, columns: CryptoTransactionFields.values);
  ...
  Future<int> delete(int id) async {
    final db = await instance.database;
    return await db.delete(tableCryptoTransaction, where: '${CryptoTransactionFields.id} = ?', whereArgs: [id]);
  }
